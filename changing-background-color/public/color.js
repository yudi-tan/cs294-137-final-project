...
function ColourSystem(name, xRed, yRed, xGreen, yGreen, xBlue, yBlue, xWhite, yWhite, gamma) {
  return { name, xRed, yRed, xGreen, yGreen, xBlue, yBlue, xWhite, yWhite, gamma };
}
...
let NTSCsystem  =  ColourSystem("NTSC",               0.67,   0.33,   0.21,   0.71,   0.14,   0.08,   ...IlluminantC,    GAMMA_REC709 ),
...
function xy_to_upvp(xc, yc, /* *up, *vp */)
{
    return [(4 * xc) / ((-2 * xc) + (12 * yc) + 3) /*up*/,
            (9 * yc) / ((-2 * xc) + (12 * yc) + 3) /*vp*/];
}
...
function gamma_correct_rgb(cs, r, g, b)
{
...
/*                          SPECTRUM_TO_XYZ
    ...
    function intensity_cb, which is called with a series of
    wavelengths between 380 and 780 nm (the argument is
    expressed in nanometers), which returns emittance at that
...
function spectrum_to_xyz(intensity_cb, adapted=false)
{
    var i;
...
function color_test()
{
    ...
    for (t = 1000; t <= 10000; t+= 500) {
        [x,y,z] = spectrum_to_xyz(bb_spectrum.bind(null, t));
        [r,g,b] = xyz_to_rgb(cs, x, y, z);
        [r,g,b] = constrain_rgb(r, g, b);
        [r,g,b] = norm_rgb(r,g,b);
        print(`  ${_t(t,5,true)} K      ${_t(x,4)} ${_t(y,4)} ${_t(z,4)}   ${_t(r,3)} ${_t(g,3)} ${_t(b,3)}`);
        stroke(0);
        colorMode(RGB,1,1,1);
        fill(r,g,b);
        x = ((t-1000)/500)%5;
        y = floor(((t-1000)/500)/5);
        ellipse(x * width/6 + width/12, y * width/6 + width/12, width/8);
    }
}

function _t(n, d, front) {
  let s = ""+Math.round(n*(10**d))/(10**d);
  if (! front && s.indexOf('.') < 0 && s.length < d+2) {
    s += ".";
  }
  if (s.indexOf('.') >= 0) {
    while (s.length < d+2) {
